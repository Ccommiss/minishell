...
/*   Updated: 2021/09/23 17:34:30 by ccommiss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static enum tokens tok[3][255] = {
	...
	[WORD] = {[' '] = TOK_WORD, ['A' ... 'Z'] = TOK_WORD, ['a' ... 'z'] = TOK_WORD, ['$'] = TOK_WORD, ['\''] = TOK_WORD, ['\"'] = TOK_WORD, [0 ... 9] = TOK_WORD, ['|'] = TOK_PIPE,
		['<'] = TOK_LESS,
		['>'] = TOK_GREAT,}
...
void tokenize(char **to_tokenize, t_token *toks)
{
	...
	while (to_tokenize[i])
	{
		...
		while (to_tokenize[i][j])
		{
			...
			if (corresp[to_tokenize[i][j]] == QUOTE && curr_corres != QUOTE) //premiere quote
			{	curr_corres = QUOTE;
				//printf ("changement de corresp \n");
				//printf (" toktype %d \n", tok[curr_corres][to_tokenize[i][j]]);
			}
			else if (corresp[to_tokenize[i][j]] == QUOTE && curr_corres == QUOTE) // quote ferme
				curr_corres = WORD;
			if (tok[curr_corres][to_tokenize[i][j]] != toks->type)
			{
				printf ("char tok actuel %d VS tok de ref %d \n ", tok[curr_corres][to_tokenize[i][j]], toks->type);
